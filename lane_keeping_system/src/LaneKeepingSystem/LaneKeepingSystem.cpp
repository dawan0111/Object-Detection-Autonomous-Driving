...
LaneKeepingSystem<PREC>::LaneKeepingSystem()
{
    ...
    mTrafficLightColorDetector = std::make_unique<GetTrafficLightColor<PREC>>();
...
void LaneKeepingSystem<PREC>::setParams(const YAML::Node& config)
{
    ...
    mMinProbability = config["DETECTION"]["CONF_THRESHOLD"].as<PREC>();
}
...
void LaneKeepingSystem<PREC>::run()
{
    ...
    while (ros::ok())
    {
        ...
        if (!mDetectTrafficSigns.empty())
        {
            ...
            const auto trafficSignId = std::get<0>(box);
            const int32_t minX = static_cast<int32_t>((std::get<1>(box) / 416.f * static_cast<PREC>(mFrame.cols)) * 0.9);
            const int32_t minY = static_cast<int32_t>((std::get<2>(box) / 416.f * static_cast<PREC>(mFrame.rows)) * 0.9);
            const int32_t maxX = static_cast<int32_t>((std::get<3>(box) / 416.f * static_cast<PREC>(mFrame.cols)) * 1.1);
            const int32_t maxY = static_cast<int32_t>((std::get<4>(box) / 416.f * static_cast<PREC>(mFrame.rows)) * 1.1);
            const auto trafficSignTime = std::get<5>(box);

            detectedTrafficSignLabel = mDetectionLabel[trafficSignId];
            
            if (detectedTrafficSignLabel == "TRAFFIC_LIGHT")
            {
                cv::Rect rect(minX, minY, maxX - minX, maxY - minY);
                cv::Mat subImage = mFrame(rect);
                trafficSignColor = mTrafficLightColorDetector->detectTrafficLightColor2(subImage);
                std::cout << "DETECT COLOR: " << trafficSignColor << std::endl;
            }
        } else {
        ...
        if (isStop)
        {
            ros::Duration delta_t = now - stopTime;
            runUpdate = false;
            if (detectedTrafficSignLabel == "TRAFFIC_LIGHT") {
                if (trafficSignColor == 3) {
                    isStop = false;
        ...
        if (mLidarDetectBox.size() > 0)
        {
            ...
            for (auto bbox : mLidarDetectBox) {
                PREC y = (std::get<3>(bbox) + std::get<1>(bbox)) / 2.f;                
                if (std::abs(std::get<0>(bbox)) <= 0.17 && std::abs(y) <= 0.08) {
                    std::cout << "?!: " << std::get<0>(bbox) << ", " << std::abs(y)  << std::endl;
                    dynamicStop = true;
                    runUpdate = false;
                    break;
                }
            ...
            PREC x = (std::get<2>(bbox) + std::get<0>(bbox)) / 2.f;
            PREC y = (std::get<3>(bbox) + std::get<1>(bbox)) / 2.f;
            PREC shortPointRadian = std::atan2(mLidarDistanceThreshold - x, y);
            PREC PI_HALF = PI / 2.f;

            if(!dynamicStop && detectedTrafficSignLabel == "SMALL") {
                // RIGHT LEFT
        ...
        if (
            enableStopDetected &&
            stopDetected &&
            !previousStopDetected &&
            (
                detectedTrafficSignLabel == "CROSSWLAK" ||
                detectedTrafficSignLabel == "STOP" ||
                (detectedTrafficSignLabel == "TRAFFIC_LIGHT" && trafficSignColor == 1)
            )
        )
        {
...
void LaneKeepingSystem<PREC>::trafficSignCallback(const yolov3_trt_ros::BoundingBoxes& message)
{
    ...
    ros::Time now = ros::Time::now();
    std::vector<std::tuple<int32_t, int16_t, PREC, PREC, PREC, PREC, ros::Time>> trafficSigns;
    ...
    for (auto boundingBox : message.bounding_boxes)
    {
        ...
        int16_t boxClass = boundingBox.id;
        int32_t boundingBoxArea = (maxX - minX) * (maxY - minY);

        if ((boundingBoxArea <= mMinBoundingboxArea && mMinProbability <= boundingBox.probability) || boxClass == 6)
        {
            continue;
        }

        trafficSigns.push_back(std::make_tuple(boundingBoxArea, boundingBox.id, minX, minY, maxX, maxY, now));
    }
    ...
    if (mDetectTrafficSigns.empty())
    {
        ...
        {
            const auto boxArea = std::get<0>(trafficSign);
            const auto label = std::get<1>(trafficSign);
            const auto minX = std::get<2>(trafficSign);
            const auto minY = std::get<3>(trafficSign);
            const auto maxX = std::get<4>(trafficSign);
            const auto maxY = std::get<5>(trafficSign);
            const auto time = std::get<6>(trafficSign);

            DetectTrafficSigns.push(std::make_pair(
                boxArea,
                std::make_tuple(label, minX, minY, maxX, maxY, time)
            ));
        }
    }
    else
    {
        while (!mDetectTrafficSigns.empty())
        {
            bool isUpdate = false;
            const auto [boundingBoxArea, box] = mDetectTrafficSigns.top();
            const auto trafficSignLabel = std::get<0>(box);
            const auto minX = std::get<1>(box);
            const auto minY = std::get<2>(box);
            const auto maxX = std::get<3>(box);
            const auto maxY = std::get<4>(box);
            const auto trafficSignTime = std::get<5>(box);
            ...
            for (auto trafficSign : trafficSigns)
            {
                const auto cBboxArea = std::get<0>(trafficSign);
                const auto cTrafficSignLabel = std::get<1>(trafficSign);
                const auto cMinX = std::get<2>(trafficSign);
                const auto cMinY = std::get<3>(trafficSign);
                const auto cMaxX = std::get<4>(trafficSign);
                const auto cMaxY = std::get<5>(trafficSign);
                const auto cTime = std::get<6>(trafficSign);

                if (std::abs(boundingBoxArea - cBboxArea) <= boundingBoxArea * 0.3 && trafficSignLabel == cTrafficSignLabel)
                {
                    DetectTrafficSigns.push(std::make_pair(
                        cBboxArea,
                        std::make_tuple(cTrafficSignLabel, cMinX, cMinY, cMaxX, cMaxY, cTime)
                    ));
                    isUpdate = true;
                    break;
                }
            ...
            {
                DetectTrafficSigns.push(
                    std::make_pair(
                        boundingBoxArea,
                        std::make_tuple(trafficSignLabel, minX, minY, maxX, maxY, trafficSignTime)
                    )
                );
            }
        }
